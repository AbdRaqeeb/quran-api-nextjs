...
import { addToCache, getFromCache } from '@/lib/redis';
...
async function getChapters(req: NextApiRequest, res: NextApiResponse<{ chapters: Chapter[] } | { message: string; }>) {
  try {
    ...
    if (useCache) {
      const foundInCache = await getFromCache(CacheKeys.CHAPTERS) as Chapter[];
    ...
    if (!chapters.length) {
      ...
      addToCache(CacheKeys.CHAPTERS, chapters, { expiry: 60 })
          .then(() => logger.info({}, '[getChapters] data added to cache'));
    }
