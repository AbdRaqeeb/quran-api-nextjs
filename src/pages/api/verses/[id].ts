...
async function getVerses(req: NextApiRequest, res: NextApiResponse<VerseResponse | { message: string }>) {
    try {
        ...
        if (!id || isNaN(Number(id))) {
            return res.status(400).json({ message: 'Please specify valid number as chapter id in params' });
        }
        ...
        if (!result.verses.length) {
            const res = await fetch(`https://api.quran.com/api/v4/verses/by_chapter/${id}?language=en&words=true&page=${page}`);

            result = await res.json() as VerseResponse;

            // add to cache but do not await to delay api response
            // this is non blocking
            addToCache(key, result, { expiry: 60 })
                .then(() => logger.info({}, '[getVerses] verses added to cache'));
        }
